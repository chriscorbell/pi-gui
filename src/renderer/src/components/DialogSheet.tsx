  ...
  return (
    <Sheet open onOpenChange={(o) => !o && cancel()} title={dialog.title} width={dialog.method === "editor" ? 640 : 440}>
        ...
        <div className="flex flex-col gap-1">
          {dialog.options.map((o) => (
            <button
              ...
              className="rounded-md border border-border px-3 py-2 text-left text-[13px] transition-colors hover:bg-hover"
            >
              {o}
            </button>
          ))}
      ...
      {dialog.method === "confirm" && (
        <div>
          {dialog.message && <p className="selectable mb-4 whitespace-pre-wrap text-[13px] text-fg-muted">{dialog.message}</p>}
        ...
        >
          {dialog.method === "input" ? (
            <input
              ...
              className="h-9 w-full rounded-md border border-border-strong bg-bg-sunken px-2.5 text-[13px] text-fg placeholder:text-fg-faint focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
            />
          ) : (
            <textarea
              autoFocus
              ...
              onChange={(e) => setText(e.target.value)}
              rows={14}
              className="w-full resize-y rounded-md border border-border-strong bg-bg-sunken p-2.5 font-mono text-[12px] text-fg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
            />
          )}
