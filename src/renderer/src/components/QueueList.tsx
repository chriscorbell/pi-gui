  ...
  return (
    <div className="anim-fade-up mb-2 flex flex-col gap-1">
      {queue.steering.map((m, i) => (
        <div key={`s${i}`} className="flex items-center gap-2 rounded-md border border-border bg-bg-sunken px-2.5 py-1.5 text-[12.5px]">
          <Zap className="h-3.5 w-3.5 shrink-0 text-warn" strokeWidth={2} />
          <span className="min-w-0 flex-1 truncate">{m}</span>
          <span className="text-[10.5px] uppercase tracking-wide text-fg-faint">steering</span>
      ...
      {queue.followUp.map((m, i) => (
        <div key={`f${i}`} className="flex items-center gap-2 rounded-md border border-border bg-bg-sunken px-2.5 py-1.5 text-[12.5px]">
          <Clock className="h-3.5 w-3.5 shrink-0 text-fg-faint" strokeWidth={2} />
          <span className="min-w-0 flex-1 truncate">{m}</span>
          <span className="text-[10.5px] uppercase tracking-wide text-fg-faint">follow-up</span>
