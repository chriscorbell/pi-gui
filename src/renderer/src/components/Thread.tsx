    ...
    return (
      <div className="flex h-full flex-col">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-3 pb-16 text-center">
          <div className="text-[24px] font-medium tracking-tight">What should we build?</div>
          <div className="max-w-[340px] text-[13.5px] text-fg-muted">Pick a session in the sidebar, start one in a project, or open a folder.</div>
  ...
  const composerBlock = (
    <>
      {widgetsAbove.length > 0 && (
        <div className="selectable mb-2 rounded-md border border-border bg-surface px-3 py-2 font-mono text-[12.5px] leading-relaxed whitespace-pre-wrap text-fg-muted">
      ...
      {live?.crashed && (
        <div className="mx-6 flex items-start gap-3 rounded-lg border border-danger/30 bg-danger-soft px-4 py-2.5 text-[13.5px]">
          ...
          <div className="selectable min-w-0 flex-1">
            ...
            <pre className="mt-1 max-h-24 overflow-auto font-mono text-[12px] whitespace-pre-wrap text-fg-muted">{live.crashed}</pre>
          </div>
